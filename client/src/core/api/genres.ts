...
export const genresApi = {
  getAll: async (): Promise<Genre[]> => {
    const response = await apiClient.get<Genre[]>('/admin/genres');
    return response.data;
  },

  create: async (data: CreateGenreRequest): Promise<Genre> => {
    const response = await apiClient.post<Genre>('/admin/genres', data);
    return response.data;
  },

  update: async (id: string, data: UpdateGenreRequest): Promise<Genre> => {
    const response = await apiClient.put<Genre>(`/admin/genres/${id}`, data);
    return response.data;
  },
