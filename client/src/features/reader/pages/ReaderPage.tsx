...
export const ReaderPage = () => {
  ...
  useEffect(() => {
    ...
    setViewCount(currentChapter.viewCount ?? null);

    if (viewTimerRef.current) clearTimeout(viewTimerRef.current);

    viewTimerRef.current = setTimeout(async () => {
      try {
        const res = await apiClient.post<{ viewCount: number }>(
          `/chapters/${currentChapter.id}/view`
        );
        ...
        <header className="mb-12 text-center">
          ...
          {viewCount !== null && (
            <p className="mt-3 text-sm opacity-40 flex items-center justify-center gap-1">
              ...
              {viewCount.toLocaleString('uk-UA')}
            </p>
          )}
        </header>
